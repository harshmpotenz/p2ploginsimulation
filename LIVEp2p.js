...
let threedPreviewTimer = null;
function triggerThreeDPreview() {
  if (threedPreviewTimer) clearTimeout(threedPreviewTimer);

  threedPreviewTimer = setTimeout(() => {
    try {
      if (typeof window.threedpreview === "function") {
        window.threedpreview();
      } else if (typeof window["3dpreview"] === "function") {
        window["3dpreview"]();
      }
    } catch (error) {
      console.error("Failed to run 3D preview trigger:", error);
    }
  }, 120);
}
...
let _heic2anyLoading = false;
let _heic2anyLoadPromise = null;

/**
 * Dynamically loads the heic2any library from CDN (only loaded once, on demand).
 */
function loadHeic2Any() {
  if (_heic2anyLoaded && window.heic2any) return Promise.resolve();
  if (_heic2anyLoading && _heic2anyLoadPromise) return _heic2anyLoadPromise;

  _heic2anyLoading = true;
...
/**
 * Checks if a file is HEIC/HEIF by MIME type or extension.
 * Some browsers may report an empty MIME for HEIC files, so we also check extension.
 */
...
async function convertHeicToJpeg(file) {
  ...
  if (!window.heic2any) {
    throw new Error('heic2any library not available after loading');
  }
  ...
  // Also create a data URL for fabric.js and data-attribute usage
...
function syncThreeDPreviewForCanvasChange(event) {
  if (event?.target?.name === "centerLine") return;
  triggerThreeDPreview();
}
previewSyncEvents.forEach((eventName) => {
  canvas.on(eventName, syncThreeDPreviewForCanvasChange);
});
...
function getCappedPreviewRenderSize(originalWidth, originalHeight, displayWidth, displayHeight) {
  ...
  return {
    width: Math.max(safeDisplayWidth, Math.round(safeOriginalWidth * scale)),
    height: Math.max(safeDisplayHeight, Math.round(safeOriginalHeight * scale))
  };
}
...
function getCanvasPreviewElements() {
  return {
    container: canvas?.wrapperEl?.parentElement || document.querySelector(".canvas-panel.left-panel") || document.querySelector(".canvas-wrap"),
...
function setPrintOnSideEnabled(enabled, options = {}) {
  ...
  if (typeof triggerThreeDPreview === "function") {
    triggerThreeDPreview();
  }
}
...
function createFrontFaceSnapshotCanvas(targetWidth, targetHeight, sourceOverride = null) {
  ...
  const faceScaleX = faceCanvas.width / bounds.width;
  const faceScaleY = faceCanvas.height / bounds.height;
  const drawLeft = (objectLeft - bounds.left) * faceScaleX;
  const drawTop = (objectTop - bounds.top) * faceScaleY;
  const scaledWidth = sourceWidth * scaleX * faceScaleX;
  const scaledHeight = sourceHeight * scaleY * faceScaleY;
...
function createPreviewWrapCanvas() {
  ...
  if (!appState.isPrintOnSideEnabled) {
    const sideCoverColor = getPrintOnSideCoverColor(effectiveEdgeType);
    wrapCtx.fillStyle = sideCoverColor;
...
function setCanvasPreviewCursor(cursor) {
  ...
  if (lowerCanvas) lowerCanvas.style.cursor = cursor;
...
function clampCanvasPreviewOffsets() {
  ...
  const minOffsetX = scaledWidth <= containerRect.width ? 0 : containerRect.width - scaledWidth;
  const maxOffsetX = scaledWidth <= containerRect.width ? containerRect.width - scaledWidth : 0;
  const minOffsetY = scaledHeight <= containerRect.height ? 0 : containerRect.height - scaledHeight;
  const maxOffsetY = scaledHeight <= containerRect.height ? containerRect.height - scaledHeight : 0;
...
function handleCanvasPreviewTouchMove(event) {
  ...
  if (preview.touchMode === "previewPinch" && event.touches.length === 2) {
    ...
    zoomCanvasPreviewAtPoint(currentCenter.x, currentCenter.y, preview.scale * scaleFactor);
...
uploadInput.addEventListener("change", async function (e) {
  ...
  // Also allow by extension if MIME type is empty (some browsers don't set it for HEIC)
  const isAllowed = allowedTypes.includes(file.type) || isHeicFile(file);
  ...
  // ============================================
  // HEIC CONVERSION: convert to JPEG if needed
  // ============================================
  let processFile = file;
  let processDataUrl = null;

  if (isHeicFile(file)) {
    try {
      // Show a loading/conversion indicator
      const loadingEl = document.getElementById("canvasLoading");
      if (loadingEl) loadingEl.classList.add("active");

      const { blob, dataUrl } = await convertHeicToJpeg(file);

      // Create a new File-like blob for the API upload
      processFile = new File([blob], file.name.replace(/\.hei[cf]$/i, '.jpg'), {
        type: 'image/jpeg'
      });
  ...
  function handleImageDataUrl(imageDataUrl) {
    const mainImgWrap = document.querySelector(".main-img-wrap");
    if (mainImgWrap) {
      mainImgWrap.setAttribute("data-main-img", imageDataUrl);
    }

    requestCanvasZoomRecalc();

    fabric.Image.fromURL(imageDataUrl, function (fabricImg) {
  ...
  // If we already have a data URL from HEIC conversion, use it directly
  if (processDataUrl) {
    galleryWrapper.style.display = "none";
    uploadInputwrap.style.display = "none";
    customForm.style.display = "block";

    handleImageDataUrl(processDataUrl);
    ...
    // Normal flow for JPEG/PNG: use Image + FileReader
    const img = new Image();
    img.onload = function () {
      galleryWrapper.style.display = "none";
      uploadInputwrap.style.display = "none";
      ...
      reader.onload = function (f) {
        handleImageDataUrl(f.target.result);
      };
...
uploadInput.addEventListener("change", async (e) => {
  ...
  // For HEIC files, convert first then read
  if (isHeicFile(file)) {
    try {
